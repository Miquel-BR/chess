...
public static function cargardatossolarpanel($projecte)
	{
	$i=1;
	$aux=array("stp_b","stp_a1","stp_a2","surfacesolar","tempwinter","tempsummer","temprest");
	$query = "select superficie_solar,stp_b,stp_a1,stp_a2,temp_winter,temp_summer,temp_rest from subsistemes_solar where projecte='$projecte' ";
	$result=mysqli_query(gestio_projectesBBDD::$dbconn, $query);
	while ($row[$i] = mysqli_fetch_assoc($result)) {$aux=array("stp_b"=>$row[$i]['stp_b'],"surfacesolar"=>$row[$i]['superficie_solar'],"stp_a1"=>$row[$i]['stp_a1'],"stp_a2"=>$row[$i]['stp_a2'],"tempwinter"=>$row[$i]['temp_winter'],"tempsummer"=>$row[$i]['temp_summer'],"temprest"=>$row[$i]['temp_rest']); $i++; }
	mysqli_free_result($result);
...
{
$k=2/($m*$Ce/3600*((log(($r2/$r1),2.71828)/$u)+(1/($r2*$h))));
...
public static function iteracion_Ta1i1($Tst1s,$Ta1,$mst1,$Celiq,$Ta1e,$mdt,$Qa1,$Qaux,$P1,$Ta1i,$ma1)
		{
		$result=((($Tst1s-$Ta1)*$mst1*$Celiq)-(($Ta1-$Ta1e)*$mdt*$Celiq) + $Qa1 + $Qaux +$P1)/($ma1*$Celiq) + $Ta1i;
		return $result;
		}
public static function iteracion_Ta2i1($Tst2s,$Ta2,$mst2,$Qa2,$P2,$ma2,$Celiq,$Ta2i)
		{
		$result=(((($Tst2s-$Ta2)*$mst2*$Celiq)-$Qa2+$P2)/($ma2*$Celiq))+$Ta2i;
		return $result;
		}
