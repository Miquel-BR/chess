...
public static function cargardatossolarpanel($projecte)
	...
	$result=mysql_query($query,gestio_projectesBBDD::$dbconn);
	while ($row[$i] = mysql_fetch_assoc($result)) {$aux=array("stp_b"=>$row[$i]['stp_b'],"surfacesolar"=>$row[$i]['superficie_solar'],"stp_a1"=>$row[$i]['stp_a1'],"stp_a2"=>$row[$i]['stp_a2'],"tempwinter"=>$row[$i]['temp_winter'],"tempsummer"=>$row[$i]['temp_summer'],"temprest"=>$row[$i]['temp_rest']); $i++; }
	mysql_free_result($result);
