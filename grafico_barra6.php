...
if(isset($_GET['nomarchivo'])){
	...
	$i=0; $k=0;
	$numero_fila=0; 
	$arrayTemp = array("mes","BalanceST1","Qa1","Qaux","BalanceDep1","BalanceDT","Pa1");
	while($filas[$i]!=NULL && $i<=17519){ 
		$row = $filas[$i]; 
		$sql = explode("|",$row);
		if($i>8760){
		$arrayTemp[$k]=array("mes"=>$sql[3],"BalanceST1"=>$sql[42],"Qa1"=>$sql[18],"Qaux"=>$sql[40],"BalanceDep1"=>$sql[44],"BalanceDT"=>$sql[41],"Pa1"=>$sql[19]);
		$k++;
		}
		$i++; 
		}
	...
	//leer mes
	$mes=$aux['mes'];
	$BalanceST1=$aux['BalanceST1'];
	$Qa1=$aux['Qa1'];
	$Qaux=$aux['Qaux'];
	$BalanceDep1=$aux['BalanceDep1'];
	$BalanceDT=$aux['BalanceDT'];
	$Pa1=$aux['Pa1'];
	
	if($mes==$mesant)
		{
		$Sum1=$Sum1+$BalanceST1;
		$Sum2=$Sum2+$Qa1;
		$Sum3=$Sum3+$Qaux;
		$Sum4=$Sum4+$BalanceDep1;
		$Sum5=$Sum5+$BalanceDT;
		$Sum6=$Sum6+$Pa1;
		}
...
$titulo="Energy Balance:Direct Use Tank";
